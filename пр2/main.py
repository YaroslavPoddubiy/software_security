...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle('Шифр Тритеміуса')
    ...
    def get_keys(self):
        return tuple(key for key in (self.key1.text(), self.key2.text(), self.key3.text()) if key)

    def encrypt(self):
        try:
            encrypted_text = self.trithemius_cipher.encrypt(self.textEdit.toPlainText(), self.get_keys())
    ...
    def decrypt(self):
        try:
            decrypted_text = self.trithemius_cipher.decrypt(self.textEdit.toPlainText(), self.get_keys())
