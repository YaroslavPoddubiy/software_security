...
class KeyGenerationWindow(QMainWindow):
    def __init__(self, knapsack_cipher: KnapsackCipher):
        self.knapsack_cipher = knapsack_cipher
    ...
    def __generate_keys(self):
        if self.keyLen.text():
            ...
            public_key, private_key = self.knapsack_cipher.generate_keys(int(self.keyLen.text()))

            self.publicKey.setText(','.join(str(i) for i in public_key))
            self.b.setText(','.join(str(i) for i in private_key['b']))
            self.m.setText(str(private_key['m']))
            self.t.setText(str(private_key['t']))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.file_name = ""
        self.__knapsack_cipher = KnapsackCipher()

        self.setWindowTitle('Шифрування рюкзака')

        self.choose_file_button.clicked.connect(self.choose_file)

        self.keyGenerationButton.clicked.connect(lambda: self.debug(self.show_key_window))
    ...
    def encrypt(self):
        key = [int(i) for i in self.key.text().split(",")]
        encrypted_text = self.__knapsack_cipher.encrypt(self.textEdit.toPlainText(), key)

        self.textEdit.setText(",".join(str(i) for i in encrypted_text))

    def decrypt(self):
        key = {}
        key["b"] = [int(i) for i in self.b.text().split(",")]
        key["m"] = int(self.m.text())
        key["t"] = int(self.t.text())
        text = [int(i) for i in self.textEdit.toPlainText().split(",")]
        decrypted_text = self.__knapsack_cipher.decrypt(text, key)

        self.textEdit.setText(decrypted_text)

    def show_key_window(self):
        self.key_window = KeyGenerationWindow(self.__knapsack_cipher)
